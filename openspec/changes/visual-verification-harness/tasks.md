...
- [ ] 7.1 **EDIT** the existing `README.md` at repo root — **DO NOT create or overwrite it.** The README already has a `## Testing` section (added during publication, after this task was originally written) with e2e commands, plus `## Quick start`, `## Architecture`, `## Contributing`, `## Roadmap`. Extend `## Testing` with what Phase 5/6 added: fixture/mock regeneration (`pnpm run e2e:mocks`), the browser-mode fast loop (`pnpm e2e:browser`), and how to regenerate mocks. Confirm every tool is free/open-source. Read the file before editing. Do not add a second Testing section.
