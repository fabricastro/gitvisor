...
- [x] 6.5 **Done — the probe was run.** It was correctly *not* run during apply, because that required pushing the workflow to the remote and the apply run was instructed not to push; the honest undecided state was recorded instead. After the repository was published the probe ran three times: the first two failed and each exposed a real defect (no failure screenshot; the binary loading from a dev server), and the third passed with `Running: WebKitGTK (v605.1.15) on linux`. The job is promoted from `workflow_dispatch`-only to `push`/`pull_request` on `main`. See "Linux probe resolved" below for the full sequence.
