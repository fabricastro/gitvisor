...
- [ ] 5.1 `tools/git-fixtures/src/bin/dump-mocks.rs`: serialise via `git_core::GitRepo`, keyed by Tauri command name, `{{FIXTURE_PATH}}` token substitution. `crates/git-core/examples/dump.rs` stays untouched.
- [ ] 5.2 Generate and commit `e2e/mocks/history.json`.
- [ ] 5.3 `e2e/support/mocks.ts` (loads + substitutes `{{FIXTURE_PATH}}`), `wdio.browser.conf.ts`.
- [ ] 5.4 `e2e/browser/welcome.spec.ts`.
- [ ] 5.5 Add `e2e:browser` script to `package.json`; run locally — must be green.

## Phase 6: CI [Requirement: Release Safety Verification; CI Trigger Matrix]

- [ ] 6.1 `.github/workflows/ci.yml`: `cargo test`, clippy, fmt, `pnpm build`, G1 `cargo tree` both-directions check (already-verified commands), browser-e2e, mocks-drift diff — all blocking.
- [ ] 6.2 Add a YAML/JSON validation step in `ci.yml` covering `.github/**`, `openspec/config.yaml`, `package.json`, `tsconfig*.json` (this repo shipped invalid YAML undetected before).
- [ ] 6.3 `.github/workflows/e2e-native-macos.yml`: nightly + `workflow_dispatch` + release tags; Spec A blocking, Spec B via `expect-red.sh`.
- [ ] 6.4 `.github/workflows/e2e-native-linux-probe.yml`: `workflow_dispatch`-only, Spec A only, per design D9's decision rule (embedded provider → `tauri-driver` → drop, 3 attempts each).
- [ ] 6.5 Run the probe; if it passes, add a PR/push-to-`main` WebKitGTK job with `continue-on-error: true`; if not, record the dropped-Linux decision and output in this change's folder — no fudged claim of coverage either way.
- [ ] 6.6 `scripts/release-scan.sh`: string scan (primary) + symbol scan (corroborating, never authoritative alone) per design §1.3's 4-outcome table (pass / plugin-shipped / scan-broken / inverted).
- [ ] 6.7 `.github/workflows/release.yml`: build → `release-scan.sh` (blocking) → `publish` with `needs: [scan]`; upload artifact + sha256 for provenance; no `continue-on-error` anywhere in build/scan/publish.
- [ ] 6.8 Run `release-scan.sh` against a real `--release` build and a real `--features e2e-webdriver` build locally, proving the positive control actually distinguishes absent vs present (closes unverified U5).

## Phase 7: Contributor Docs [Requirement: Contributor Commands Use Only Free, Open-Source Tooling]

- [ ] 7.1 **EDIT** the existing `README.md` at repo root — **DO NOT create or overwrite it.** Corrected by the orchestrator: `tasks.md` originally claimed no README existed. It does (4.2 KB, with `## Quick path`, `## What it does today`, `## Architecture`, `## Development`, `## Roadmap`, `## License`), and `## Architecture` carries the commit-graph lane-algorithm explanation that must survive. Extend `## Development` with the harness commands: install, fixture build, browser-mode and native e2e runs, `expect-red.sh`. Note that every tool (`@wdio/*`, `tauri-plugin-wdio-webdriver`, `git2`) is free and open-source. Read the file before editing.
- [ ] 7.2 Re-run every documented command in sequence to confirm the loop works end to end — manual verification, not a unit-test-first step (no JS/TS runner installed per `openspec/config.yaml`).
